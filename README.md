...
**P1（進行中）**：收藏 + 探索 + 加入行程 fast-path（含 direct-mode）已完成第一波；Entry CRUD 已完成 `/trips/:id/add-entry` 搜尋/收藏新增 slice 與 `/trips/:id/stop/:entryId/edit` 時間/描述/刪除 slice；建立/編輯行程、AI 聊天、全域地圖、共編邀請仍待辦。
