...
class _TripHealthScreenState extends ConsumerState<TripHealthScreen> {
  ...
  Future<void> _onRequestTerminal() async {
    ...
    // 工單終結是已知事實，不能等報告回來才記住；重新整理可能讓這次讀取作廢。
  ...
  Widget build(BuildContext context) {
    ...
    if (_isPending && requestId != null && !_requestTerminated) {
      ref.listen(requestLifecycleProvider(requestId), (previous, next) {
        if (previous is! RequestTerminal && next is RequestTerminal) {
          unawaited(_onRequestTerminal());
        }
      });
    }
