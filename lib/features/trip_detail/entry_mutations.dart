...
/// 操作成功後的共同政策；呼叫端只提供已知的 Day／entry 身分。
enum EntryMutation {
  masterSelected,
  reordered,
  copied,
...
class EntryMutations extends Notifier<TravelRecomputeState> {
  ...
  /// 唯一的 mutation 效果表：決定重算範圍與需失效的讀取。
  Future<void> record(
    EntryMutation mutation, {
    int? dayNum,
    Iterable<int> dayNums = const [],
    int? entryId,
  }) async {
    final effects = switch (mutation) {
      EntryMutation.masterSelected => (
        changed: <TripChange>{
          TripChange.days,
          TripChange.segments,
          TripChange.entry,
      ...
      EntryMutation.reordered => (
        changed: <TripChange>{TripChange.days, TripChange.segments},
        scopes: <int?>[...dayNums],
      ),
      EntryMutation.deleted || EntryMutation.added => (
        changed: <TripChange>{TripChange.days, TripChange.segments},
        scopes: <int?>[dayNum],
      ),
      EntryMutation.updated => (
        changed: <TripChange>{
          TripChange.days,
          TripChange.entry,
          TripChange.segments,
      ...
      EntryMutation.copied => (
        changed: <TripChange>{TripChange.days},
        scopes: <int?>[],
      ),
      EntryMutation.poiUpdated => (
        changed: <TripChange>{TripChange.days, TripChange.entry},
        scopes: <int?>[],
      ),
      EntryMutation.segmentChanged => (
        changed: <TripChange>{TripChange.days, TripChange.segments},
        scopes: <int?>[],
      ),
    };
    for (final scope in effects.scopes) {
      await recomputeTravel(dayNum: scope);
    }
    refreshAfter(effects.changed, entryId: entryId);
  }
  ...
  Future<bool> recomputeTravel({int? dayNum, bool auto = false}) async {
    ...
    try {
      ...
      // 次要重算即使遇到非 Exception 的平台錯誤，也不能讓已成功的寫入重送。
...
/// [ref] 可為 Ref 或 WidgetRef，兩者沒有公開的共同 invalidate 介面。
