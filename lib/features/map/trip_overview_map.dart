/// 行程總覽地圖核心(trip map 與 /map 共用):由 [TripDay] 清單萃取 per-day pins,
/// 畫 day tabs(總覽 / DAY N)＋ GoogleMap ＋ 底部 entry cards ＋ per-day 道路折線。
/// trip map 傳固定行程的 days;/map 由 trip picker 切換行程後傳選定行程的 days。
library;
...
/// 行程總覽地圖視圖:傳入已載入的 days,自行處理 day tabs / 地圖 / cards / 折線。
/// 切換行程時上層應以 `ValueKey(tripId)` 重建本 widget（重置選中日、避免 tab 越界）。
class TripDayMapView extends ConsumerStatefulWidget {
  const TripDayMapView({super.key, required this.days});

  final List<TripDay> days;

  @override
  ConsumerState<TripDayMapView> createState() => _TripDayMapViewState();
}
...
class _TripDayMapViewState extends ConsumerState<TripDayMapView> {
  ...
  @override
  void didUpdateWidget(covariant TripDayMapView oldWidget) {
