...
/// 帳號 sheet 以 query 的 `account_origin` 指定來源頁。
String accountSheetLocation(Uri uri, String page) {
  final rawOrigin = uri.queryParameters['account_origin'];
  final parsedOrigin = rawOrigin == null ? null : Uri.tryParse(rawOrigin);
  final origin =
      parsedOrigin != null &&
          !parsedOrigin.hasScheme &&
          !parsedOrigin.hasAuthority &&
          isShellContentLocation(parsedOrigin.path)
      ? parsedOrigin
      : Uri.parse('/trips');
  ...
  query['account'] = page;
  query['account_origin'] = withoutAccount(origin);
...
/// 舊路徑 → 帳號 sheet 的頁。來源頁由 `account_origin` query 指定。
...
List<GoRoute> legacyAliasRoutes() => [
  ...
  for (final MapEntry(key: path, value: page) in accountAliases.entries)
    GoRoute(
      path: path,
      redirect: (_, state) => accountSheetLocation(state.uri, page),
    ),
];
