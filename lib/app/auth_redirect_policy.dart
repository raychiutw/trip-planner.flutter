...
bool isPublicShellOutsideRoute(Uri uri) {
  if (publicShellOutsideRoutes.contains(uri.path)) return true;
  final segments = uri.pathSegments;
  return publicShellOutsideRoutes.contains('/s/:token') &&
      segments.length == 2 &&
      segments.first == 's';
}
...
String? authRedirect({
  ...
  final isOnLogin = uri.path == '/login';
  final isOnWelcome = uri.path == '/welcome';
  if (!isLoggedIn && !isOnLogin && !isPublicShellOutsideRoute(uri)) {
