...
/// 引擎與 transport 之間的 seam：同參數送一次請求，不讀寫或淘汰快取；
/// 非 2xx 丟 [ApiError]，連線層失敗丟 [OfflineSendFailure]。
typedef OfflineSend =
...
/// transport 回報的連線失敗；保留原例外供公開呼叫入口原樣拋出。
class OfflineSendFailure implements Exception {
  const OfflineSendFailure({
    required this.cause,
...
/// 管理離線佇列、樂觀快取與 OCC 衝突；只依賴送出函式與持久化快取。
class OfflineSyncEngine {
  /// [send] 由 transport 注入；引擎自行處理快取與佇列副作用。
  ...
  /// 在佇列臨界區讀 stale，並補回當機前已入隊但尚未寫入快取的 patch。
  Future<CacheEntry?> readStale(String key) => _withQueueLock(() async {
    final cached = await _store.readResponse(key);
    if (cached == null) return null;
    final patched = await _applyPendingPatches(key, cached.data);
    if (!identical(patched, cached.data)) {
      await _store.writeResponse(key, patched, cachedAt: cached.cachedAt);
    }
    return CacheEntry(data: patched, cachedAt: cached.cachedAt);
  });

  /// 將網路 fresh 與仍在佇列中的 patch 合成同一份畫面及快取資料。
  Future<Object?> reconcileFresh(String key, Object? fresh) => _withQueueLock(
    () async {
      final patched = await _applyPendingPatches(key, fresh);
      final isEmpty = patched == null || (patched is String && patched.isEmpty);
      if (!isEmpty) await _store.writeResponse(key, patched);
      return patched;
    },
  );

  /// 寫入:線上直送(成功即依失效表 evict);離線且帶 [optimistic] → 入持久化佇列
  /// + 對 op.cacheKey 套樂觀 patch + 回 null(樂觀成功)。非離線錯誤往上拋。
  Future<dynamic> sendMutation(
    ...
    return _withMutationResourceLock(optimistic.cacheKey, () async {
      ...
      try {
        final result = await _send(method, path, body: body, query: query);
        await evictForMutation(store, method, path, body);
        return result;
      } on OfflineSendFailure catch (e) {
        if (e.isOffline) {
        ...
        Error.throwWithStackTrace(e.cause, e.stackTrace);
      }
    });
  }
  ...
  Future<RebaseOutcome> _tryRebase(
    ...
    try {
      ...
      return e.isOffline ? RebaseOutcome.offline : RebaseOutcome.conflict;
    }
    ...
    if (conflictFields.isEmpty) {
      try {
        ...
        if (evictAfterSuccess) {
          await evictForMutation(store, m.method, m.path, m.body);
        }
        return RebaseOutcome.synced;
      } on ApiError {
        return RebaseOutcome.retryLater;
      } on OfflineSendFailure catch (e) {
        return e.isOffline ? RebaseOutcome.offline : RebaseOutcome.retryLater;
      }
    }
    await store.appendConflict(
  ...
  Future<Object?> _refetchFor(QueuedMutation m, String tripId) {
    if (m.type == 'entry.update') {
      return _send('GET', '/trips/$tripId/days', query: {'all': '1'});
    }
  ...
  Future<void> resolveConflictKeepOurs(ConflictRecord c) async {
    ...
    try {
      ...
      await evictForMutation(store, 'PATCH', c.path, c.body);
  ...
  Future<Object?> _applyPendingPatches(String key, Object? base) async {
