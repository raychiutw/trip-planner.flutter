...
- **停留點 web route 相容**:新增 `/trips/:tripId/entries/:eid/edit|copy|move` 全頁入口,並支援 web alias `/trip/:tripId/stop/:entryId/edit|change-poi|copy|move`；`change-poi` 地點管理可用搜尋或收藏置換正選 POI / 加入備選。
