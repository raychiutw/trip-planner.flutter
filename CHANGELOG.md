...
- 保留品牌前景、媒體背景暗化、跨平台降級及獨立的降低透明度／提高對比設定；聊天輸入列與 POI accessory 承接共用材質。

### 修正

- 補足套件公開元件的鍵盤、語意操作、大字及 Reduce Motion 整合；保留分頁狀態、日期再次選取、選單立即派發及導航生命週期。
- sheet 拖曳關閉與內層返回沿用未儲存草稿確認；修正真正 push 子頁同意捨棄後仍卡住，以及預設 sheet 高度下橫向載入畫面溢位。

### 驗證

- 完成 1,882 項自動測試及 Android 15 模擬器的 Skia debug app-owned 整合流程。新／舊 iOS 真機、原生地圖共存、Impeller 材質與持續操作效能仍待驗收，詳見 [遷移紀錄](docs/liquid-glass-1.4.1-migration.md)。
