...
- 停留點設為正選、編輯、刪除與跨日重排共用異動政策；衝突時保留使用者改過的欄位，重新排序依最新資料規劃，避免舊位置覆蓋其他人的變更。
- 停留點與筆記的離線異動在重連後沿用同一套同步與衝突處理，避免資源快取鍵不一致。
- 浮動標題與固定操作列共用帶狀遮蔽及動作寬度計算，並補齊 iOS 模擬器畫面證據。
- 行程與收藏從選單刪除時使用破壞性確認，滑動與頁面按鈕維持破壞性 alert；其他破壞性確認統一由共用入口依觸發來源呈現。
- 時間軸與地圖共用「指定 Day／全部／未指定」選取狀態；深連結日期優先於停留點，背景分支不覆寫目前選取，切回地圖仍保留「全部」。
- 登入遇 CDN 攔截頁時顯示上游暫時不可用，429 保留 `Retry-After`；帳密 POST 不自動重送。
- 聊天工單回到前景時會補讀最新結果；停止等待後即使伺服器未確認或回覆補讀遇到登入過期，畫面仍能脫離思考中並如實提示。
- 聊天收到完成事件後若回覆補讀暫時失敗，會繼續等待可用回覆，不再留下空白的 AI 訊息。

### 改善

- 停留點操作集中管理確認、資料失效及交通重算，時間軸與地點畫面使用相同規則。
- 離線佇列、重連同步與 OCC 衝突處理收進 `OfflineSyncEngine`；行程與筆記的樂觀操作改以資源產生快取鍵。
...
- 行程日刪除後的確認查詢明確只採網路結果；一般行程讀取與不留存的敏感狀態各自使用一致的快取政策。

## [0.29.6] - 2026-09-25

### 修正

- 切換行程時可直接清除搜尋並恢復完整清單，保留目前行程與鍵盤；搜尋清除動作提供朗讀名稱，無結果提示支援動態朗讀，最大字級仍可清除並選取行程。

## [0.29.5] - 2026-09-25

### 修正

- 編輯行程儲存時持續顯示進度並避免重複送出；延遲成功保留後續輸入，失敗保留草稿供重試。畫面明示欄位需儲存，Day 新增、刪除與日期平移立即生效，取消欄位草稿不會還原已完成的 Day 操作。

## [0.29.4] - 2026-09-25

### 改善

- 登入裝置頁清楚說明目前無法一次登出其他裝置，並引導返回裝置清單逐一確認登出。

## [0.29.3] - 2026-09-25

### 修正

- 行程筆記在回前景、停止等待與重試時保留正確的 AI 工作；舊進度與摘要不覆蓋新工作，摘要讀取失敗也不抹掉已確認的完成或逾時狀態。
- 住宿生成的排除清單可直接開啟與逐項恢復，與一般生成各自獨立；窄螢幕與放大字級下的入口會換列。

## [0.29.2] - 2026-09-25

### 修正

- 新增行程送出中暫停編輯並持續顯示進度，避免連點或成功後的新輸入重複新增；失敗保留全部草稿供重試，成功只開啟後端回傳的行程。

## [0.29.1] - 2026-09-25

### 修正

- 異動紀錄明確說明顯示最近最多 50 筆，即使紀錄較少或尚無紀錄，也能辨識資料範圍；大字級下可完整閱讀說明。

## [0.29.0] - 2026-09-25

### 改善

- 公開分享、列印預覽與 PDF 顯示相同的五區筆記內容，包含航班艙等；保留原文複製、欄位閱讀標籤、電話／郵件與安全網址連結，開啟失敗可重試。
- 長篇中文筆記可在畫面捲動閱讀並於 PDF 跨頁；空白連結標籤以目的地提供可辨識名稱，公開輸出只使用已授權內容。

### 修正

- 公開分享區分失效連結與暫時載入失敗，支援原地重試；聊天行程清單載入失敗重試時保留既有草稿。
...
- 加入行程時，行程或日期讀取失敗可各自原地重試，保留有效選擇與自訂起訖時間；重試中顯示進度並避免連按造成重複讀取，切換行程後不受舊行程的完成結果干擾。
- 重新載入後，已移除的行程或日期會改回有效選項，空清單不能提交；取消選擇或重選相同項目不會把表單誤判為已修改。
- 長行程名稱與日期改由可捲動的選擇面板完整呈現；加入行程與停留點編輯的時間欄位在大字級下會換列，避免控制項超出畫面。

## [0.28.3] - 2026-09-25

### 修正

- 公開分享頁區分失效連結與暫時網路錯誤，載入失敗可原地重試；大字級錯誤說明可捲動操作，重試等待中不重複送出。
- 聊天行程清單載入失敗時，即使正在自動重試仍保留手動恢復入口；已有對話、各行程草稿、游標與組字狀態會留在畫面上。
- 聊天重試進行中會合併重複操作，讀取結束或再次失敗後可繼續操作；快取先到時仍等待背景讀取完成。

## [0.28.2] - 2026-09-25

### 修正

- 註冊、忘記密碼與設定新密碼欄位補上系統 AutoFill 設定；註冊與密碼更新成功後才通知系統保存憑證，註冊保存不再等待驗證信寄送；失敗保留輸入，離頁後晚到的結果不會保存新表單或改變目前頁面。
- 密碼重設連結失效時提供「重新申請」入口；信箱驗證連結失效或不完整時提供「重新開始」，一般連線或服務錯誤則可保留原連結重試。開啟新的重設密碼或驗證連結會清除前一個連結的畫面狀態。

## [0.28.1] - 2026-09-25

### 修正

- 停留點編輯、複製、移動與 POI 深連結會先驗證 ID；無效值顯示可返回行程列表的說明畫面，不再拋出解析錯誤或請求錯誤的停留點。
- 舊行程地圖連結會開啟 root「地圖」分頁，保留行程、已選 Day 與停留點，不另開地圖子頁。

## [0.28.0] - 2026-09-25

### 修正

- 個人資料儲存期間繼續輸入的新名稱會保留；先前的名稱儲存成功後，不會蓋掉新草稿或自動離頁。重複送出共用同一次儲存，失敗可保留輸入重試；成功或失敗後都保留游標與中文組字狀態。
- 個人資料載入失敗可原地重試，儲存進度與錯誤會向輔助使用工具宣告；離頁後完成的儲存仍會觸發認證狀態重新載入。

## [0.27.0] - 2026-09-24

### 新增

- 行程健檢回到 App 前景時會補讀工單狀態；停止等待未獲伺服器確認時保留清楚提示與既有報告查詢。

### 變更

- 帳號、定位、浮動標題、固定列、根分頁列與日期選擇器共用導覽玻璃的材質、前景及輔助使用降級決策；保留各控制項原有的幾何、操作與選取行為。
- 移除未使用的舊玻璃相容入口，補上跨畫面與鍵盤、讀屏、提高對比及降低透明度的回歸驗證。
- 共用標題組裝延續既有的媒體背景白色前景，避免重構時讓淺色地圖標題退回黑字。

## [0.26.10] - 2026-09-15

### 修正

- 深色行程操作選單改用與底部分頁列相同的玻璃底色，減少偏灰白的白膜；保留背景模糊、快捷動作排列、刪除確認及輔助使用降級。

## [0.26.9] - 2026-09-11

### 修正

- 修正淺色地圖的行程標題顯示黑字而難以閱讀；標題文字沿用容器前景色，並保留深色模式、降低透明度與提高對比的配色。

## [0.26.8] - 2026-09-11

### 修正

- 依真機前後對照降低共用導覽玻璃的主要高光強度，收斂深色標題、按鈕與分頁列的環形亮邊；保留選單可讀性、帳號表面層次與輔助使用降級。

### 驗證

- 依裝置紀錄調整整組測試的執行預算：iOS 七分鐘、Android Patrol 十五分鐘，Android 工作上限六十分鐘；保留標準 integration 五分鐘、各操作時限、所有測試目標與發布保護。新的高光設定仍待合併後真機複驗。

## [0.26.7] - 2026-09-11

### 修正

- 加強選單背景的模糊與中性底色，減少後方卡片標題干擾閱讀；收斂玻璃邊緣高光，保留提高對比與降低透明度的獨立降級。
- 帳號面板採有層次的系統語意表面，讓深色模式的面板與原頁保持區隔，延續既有導覽與關閉保護。

### 驗證

- Android 裝置流程在搜尋後等待鍵盤收合、分頁列恢復再切換，保留原有搜尋結果與聊天草稿檢查，並以延遲鍵盤回歸測試防止相同時序錯誤。

## [0.26.6] - 2026-09-11

### 變更

- 行程卡點擊「⋯」與長按開啟同一份錨定玻璃選單，上排提供分享、共編與 AI 健檢；下方保留匯出及分組刪除。收藏、探索分類、筆記與其他動作選單統一圖示、文字及入口樣式，內容列的「⋯」一般狀態不顯示白框。
- 帳號面板從下方開啟，手機上的展開高度依狀態列安全區域調整；保留子頁返回、關閉保護與原頁狀態。
- 移除聊天輸入列的「＋」及其選單，輸入框向左延伸；語音、送出、草稿與時間軸新增停留點維持原有操作。

### 修正

- 快捷動作文字放大後改為直列，避免臨界字級裁切；自訂分類入口維持正確錨點，減少動態效果時不縮放快捷動作，選單語意名稱保持獨立並宣告展開狀態。
- 修正清單排序後選單入口失效、Android 返回鍵直接離頁，以及鍵盤收起時剛開啟的選單立即關閉；選單在關閉動畫中重開仍可由返回鍵關閉，長清單也會避開底部安全區域。

### 驗證

- 補上筆記交還維護失敗後保留內容、持續顯示錯誤與重開選單的回歸測試；release 流程以公開語意樹驗證 Day 選取狀態。
- 新增既有真機測試入口的玻璃視覺流程，涵蓋真實地圖、帳號面板、Light／Dark 與獨立輔助使用狀態；實際裝置材質、動畫與商店處理結果待合併後驗證，不以 host 測試代替。

## [0.26.5] - 2026-09-10

### 修正

- 地圖帳號與定位按鈕改用更通透的共同玻璃底，保留清楚的圖示與既有日期列；定位中也維持提高對比／降低透明度的不透明降級與停用保護。

## [0.26.4] - 2026-09-10

### 修正

- 地圖上未選取的日期與右上帳號圖示，在亮色圖磚上也能清楚辨識；保留半透明背景、選取日期的品牌色，以及提高對比／降低透明度設定。

## [0.26.3] - 2026-09-09

### 變更

- 將 `liquid_glass_widgets` 固定升級至 1.4.1，採用公開預設材質；root tab、日期選擇器、導覽群組、帶狀遮蔽、選單與 sheet 的繪製及幾何交由套件接手，移除舊光學校準與自畫選取效果。
- 保留品牌前景、媒體背景暗化、跨平台降級及獨立的降低透明度／提高對比設定；聊天輸入列與 POI accessory 承接共用材質。媒體表面進入不透明降級時，文字改用對應的系統前景色，避免淺底白字；sheet 返回圖示保留品牌色。

### 修正

- 補足套件公開元件的鍵盤、語意操作、大字及 Reduce Motion 整合；保留分頁狀態、日期再次選取、選單立即派發及導航生命週期。自訂選單入口補上提示，修正時間軸選單關閉途中以讀屏長按重開後持續排幀的問題。
- sheet 拖曳關閉與內層返回沿用未儲存草稿確認；修正真正 push 子頁同意捨棄後仍卡住、預設 sheet 高度下橫向載入畫面溢位，以及探索自訂地區 sheet 離場時輸入 controller 過早釋放。
- 開啟「減少動態效果」時，sheet 放手後直接停在套件選定的位置，不再繼續彈動、縮放或拉伸；旋轉螢幕後依新尺寸重新定位，保留草稿、鍵盤與關閉保護。
...
- 底部 tab bar 的帶狀遮蔽模糊改與頂部同一套(前 55% 全層、後 45% 逐層縮短);淡出曲線不變。合併前需真機目視(ADR-0001)。

### 內部

- 頂部與底部帶狀遮蔽併成一個 widget;固定 bar 的 large sheet 路徑與一般路徑共用動作寬度;動作群組改走 `TpGlassSurface`。

## [0.25.5] - 2026-09-05

### 修正

- 地圖分頁沒有行程、載入中或載入失敗時,標題列不再用地圖背景的白字與暗化層畫在淺色底上。

### 內部

- 「在媒體背景上」改由 `TpMediaBackdropScope` 宣告一次,浮動 header、帶狀遮蔽、底部 accessory、root tab bar 各自讀取,不再手傳 bool 或用分頁索引猜。

## [0.23.1] - 2026-08-25

### 修正

- 帳號頁「外觀」列的目前選項與右側箭頭改為靠右對齊,不再往左縮排、與同群組其他列的箭頭對不齊。
- 外觀頁未選中的選項不再顯示右側箭頭,只有目前選中的模式顯示勾選標記。

## [0.23.0] - 2026-08-20

### 新增

- 可在「帳號 > 偏好 > 外觀」切換跟隨系統、淺色或深色；選擇會立即生效並保存在裝置上，重新啟動後仍會沿用。
- 既有 `/account/appearance` 與 `/settings/appearance` 深連結會直接開啟外觀頁，同時保留登入保護與原本的返回位置。

## [0.22.0] - 2026-08-03

### 移除

- 行程時間軸不再顯示天氣示意與逐時預報，也不再向 Open-Meteo 查詢天氣資料。

## [0.21.0] - 2026-07-29

### 新增

- AI 健檢進行中也能停止等待,那一頁不會永遠停在「進行中」。另外,如果健檢的請求已經被系統收掉、而報告卻還停在進行中(伺服器端的已知限制),畫面現在說得出「這次健檢已經停止,報告不會再更新了」並讓你重新健檢 —— 不再讓人一直重整等一個不會來的結果。

- 筆記 AI 生成等太久時也能停止等待。停一種不會連坐另一種正在跑的 —— 停「一般行前須知」時,「緊急聯絡」照樣繼續。同樣**不中止 AI**:停止之後若 AI 仍完成了,結果還是會寫進筆記。

- 聊天等太久時可以在「思考中…」泡泡上按「停止等待」,不必再看著轉圈。**它不會中止 AI** —— 按下去只是不再等,AI 若仍在處理,完成後的回報還是會出現在對話裡,行程也可能已被更動;文案照實這樣寫,不會假裝成「已中止」。

### 變更

- 聊天請求結束時說得出**為什麼**結束:自己停的、系統逾時自動停的、還是需要行程擁有者授權。先前無論哪一種都只有一句「AI 處理失敗」,使用者不知道該重送還是該去要授權。

## [0.20.0] - 2026-07-28

### 變更

- 改壞了一則原本由 AI 維護的筆記,可以長按把它交還給 AI。交還當下內容一字不變,要等下一次生成才更新 —— 這不是一個會嚇到人的動作。純手動建立的筆記根本不會出現這個選項(不是出現但不能點)。
- 刪掉的 AI 建議不會自己長回來,而且救得回來。行前須知與緊急聯絡的標題列會顯示「已排除 N 項」,點開看得到刪掉的是什麼、什麼時候刪的,每一則都能恢復。沒有被排除的項目時入口不出現,版面一如現狀。恢復改的是「下次會不會生成」而不是當下的內容 —— 那一則要等下一次生成才會再出現。

- 行前須知與緊急聯絡的每一列,目前由 AI 維護的會標上「AI」。標記看的是**現在誰維護**而不是當初誰產生 —— 你親手改過那一列之後標記就消失,代表它下次生成不會被覆蓋。標記走中性語意層不另外調色,讀螢幕念得到「AI 產生」。欄位缺漏時一律當成人工維護(安全側):誤判成 AI 會讓手寫內容在下次生成被蓋掉。

- 生成結束時會用中文句子講清楚動了什麼:「這次生成新增 2 則、替換 5 則、保留你手動維護的 3 則。」為零的項目直接略過,不會出現「抑制 0 則」這種雜訊。摘要是在生成結束後重讀一次狀態拿到的(進度通道本身不帶這些數字),不是輪詢。
- 生成逾時有自己的面板與重試,不再跟一般失敗混為一談。逾時與失敗在進度通道上長得一樣,靠結束後重讀狀態才分得出來 —— app 不自己倒數,一律以伺服器為準。

- app 被系統回收後重開、或在手機按了生成再用平板打開同一個行程,看到的都是同一個進行中的狀態,不會以為失敗而重按一次。回到前景時會重讀一次生成狀態並接上還在跑的工作;已經接上的不會重接,所以與離線佇列同步同時發生也不會把同一個工作的事件收兩次。

- 按下 AI 生成後離開筆記頁,回來仍看得到它在跑。進頁面時讀一次持久狀態,發現有進行中的工作就直接接上既有的進度通道 —— **不新增任何輪詢**,狀態只讀這一次。先前進行中狀態只活在畫面的記憶體裡,離開就消失,回來看到的是「什麼都沒發生」,但伺服器上那個 3-7 分鐘的工作還在跑、跑完也沒人通知。離開頁面只是停止盯著,不會取消伺服器上的工作。
