...
- 取消收藏的文案改為「移除」(對齊詞彙表:收藏是可重新建立的關聯);確認對話框、左滑與長按動作、編輯模式按鈕與提示一併調整,樣式維持破壞性,警語保留「無法復原,只能重新收藏」。

## [0.25.9] - 2026-09-07

### 內部

- 依 ADR-0008 稽核全部確認對話框:十個刪除確認改走 `showAppDestructiveConfirm`,樣式不變;新增守門測試強制破壞性確認一律經 wrapper。
