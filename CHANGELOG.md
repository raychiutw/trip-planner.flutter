...
- AI 工單等待:事件串流開著但久無訊息時自動改為輪詢;登入過期、無權限或工單不存在時直接結束等待,不再每隔幾秒打空槍;工單結束後以伺服器的最終狀態為準(聊天的「需要授權」靠這個才拿得到)。

### 變更

- 健檢工單結束後報告讀取失敗時,顯示「健檢已結束,但報告讀取失敗」與重試鈕,不再誤標成停滯;重試只重讀報告表。

## [0.24.0] - 2026-09-05

### 新增

- 新增 `RequestLifecycle`:AI 工單的等待流程(種子讀取 → SSE → 斷線改輪詢 → 終結)集中在一個 module,畫面只看狀態與「停止等待」。健檢畫面先遷移;聊天與筆記後續遷移。
- 輪詢改為 4 秒起每輪加倍、30 秒封頂,app 進背景暫停、回前景補讀一次後重設;同一時間只有一個讀取在飛。

### 變更

- 健檢畫面 app 回前景會補讀工單狀態;工單終結後重讀報告表,仍 pending 才標為停滯。
- 健檢「停止等待」送出後按鈕先停用,連點不會送第二次;伺服器先一步終結時保留伺服器的結果,不再誤報「伺服器沒有確認」。
- 健檢重新整理不再清掉同一張工單的停滯判定,停止等待後按重新整理不會退回「進行中」。
