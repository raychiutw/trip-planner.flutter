...
- **P0 行程清單 parity**:TripsList 新增分類 tabs（全部/我的/共編/已歸檔）、搜尋、排序（最新編輯/出發日近/名稱）、filtered empty、TripCard action menu（編輯/共編/AI 健檢/筆記/刪除）、尾端新增卡與 JSON 匯入（512KB/schemaVersion 1 檢查）；`TripSummary` 解析 owner/role/countries/start/end/updated/member/archive 等 `/my-trips` rich fields。
