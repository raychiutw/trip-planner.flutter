...
- **新增停留點 deep link**:新增 `/trips/:tripId/entries/new?day=N` 全頁新增入口,支援搜尋 POI、收藏景點或自訂停留點；web alias `/trip/:tripId/add-entry|add-stop|add-custom-stop?day=N` 會進對應模式,`/trip/:tripId/add-stop?tab=favorites&day=N` 會進收藏景點模式,搜尋模式會保留 `region` query 並送進 POI 搜尋；搜尋/收藏模式皆支援 web 相容分類篩選。
