...
- 住宿建議生成的排除清單在筆記畫面可查看與逐項恢復(之前只記數量、看不到內容)。排除清單入口放在生成按鈕旁,同一區多於一種生成有排除時會標出是哪一種。

### 變更

- 排除清單關掉後會重讀 AI 狀態,恢復後「已排除 N 項」的數字跟著變。
- AI 生成類型的所在區、前置條件、API 路徑與顯示文字集中在型別本身,新增一種生成類型只改一處。
