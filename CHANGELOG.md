...
- 登入遇到 CDN 攔截頁時顯示「伺服器暫時無法回應」,不再誤報登入回應異常。
- AI 事件串流遇到限流時先收掉回應再等待重送,等待期間不佔著連線。
- 登入 / 註冊送出後不再做任何重送:密碼打錯不會觸發 token refresh。

### 內部

- 「同參數重送一次」的決策抽成純函式 `decideRetry`,一般請求與事件串流共用同一份規則,消除兩份複本的漂移。
