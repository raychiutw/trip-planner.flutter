...
- 地圖資料重新載入(離線同步、編輯後)時不再跳回原本的那一天,也不再把跳回寫進時間軸;地圖與時間軸來回切換兩次以上仍保持同一天;地圖分頁換行程時優先用時間軸已為該行程選的那一天。

## [0.25.3] - 2026-09-05

### 修正

- 地圖選「全部」後,時間軸捲到某一天再切回地圖仍維持「全部」;重新開啟地圖時共用選取日是「全部」就直接顯示「全部」。
- 時間軸捲動換天時不再整頁重建。

### 內部

- 時間軸與地圖共用的「當前 Day」改為三態(指定某天 / 全部 / 未指定)並綁行程;停留點所在天與行程標題的查找收進共用函式。
