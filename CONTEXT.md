...
**工單**:
一次 AI 請求在後端是一筆工單,由外部 worker 非同步處理後回填結果。不是對話式 API,沒有多輪記憶;一張工單等於一組請求與回覆的交換。per-trip,同行程的協作者共享同一串。聊天訊息、筆記生成、行程健檢三種畫面都會產生工單,等待流程共用同一份(工單 lifecycle)。
_避免_: 行程請求、對話訊息、chat message、prompt

**思考中**:
工單尚未回填答覆時顯示的 AI 占位(聊天氣泡、筆記與健檢的等待狀態皆屬此)。等待可長達數十分鐘,不設短 timeout 放棄。
_避免_: loading、pending reply

**停止等待**:
使用者主動放棄等待工單結果,只讓畫面脫身,**不中止 AI**——工單若仍在處理,後端完成後結果仍可能寫入。伺服器沒有確認停止時,畫面照樣本機終結並誠實提示可能仍在處理,不假裝已中止。
_避免_: 取消、cancel、abort

**停滯**:
工單已終結(含被系統牆鐘收掉)但對應資料仍卡在 pending、未回填的落差狀態。畫面要能辨識並提示,不能無限等待,也不能被之後的重新整理誤判回「進行中」。
_避免_: stuck、hang、逾時
