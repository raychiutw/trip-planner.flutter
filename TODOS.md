...
- [x] 設定子頁:**外觀、個人資料、通知設定 toggle、登入裝置、已連結 OAuth app、開發者 OAuth app 已完成**。
- [x] OAuth PKCE + Bearer 認證:**client 端、登入按鈕、consent 頁、connected/developer app 管理已完成 + 單測**(pkce/token/refresh/Bearer ApiClient/loopback 編排),production backend 已 provision `tripline-mobile` active public client。實機 browser/loopback e2e 仍依 `docs/howto-oauth-pkce.md` 手動跑。
...
- [x] 地圖介面抽象化與原生 Google Maps 遷移:`TripMapScreen`/`GlobalMapScreen` 皆走 `features/map/map_adapter.dart`，SDK 型別集中在 adapter 內(**Completed:** v0.6.0, 2026-07-15)
...
- [x] P2 通知設定頁:`/settings/notifications`、`/account/notifications` route + backend preferences GET/PATCH toggle(**Completed:** 2026-07-09)
- [x] 地圖 adapter:`TripMapPoint`/`GoogleTripMapCanvas`/`GoogleTripMapController` 集中原生 Google Maps、marker、camera 與 route polyline 轉接(**Completed:** v0.6.0, 2026-07-15)
- [x] 全域地圖(map tab 轉正):收藏 POI 以原生 Google Maps 跨行程呈現(依 poi_type 上色 + 點選資訊卡)(**Completed:** v0.6.0, 2026-07-15)
