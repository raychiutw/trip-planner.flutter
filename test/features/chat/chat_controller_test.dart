...
void main() {
  ...
  ChatController ctrlOf(ProviderContainer c) {
    c.listen(chatControllerProvider('t'), (_, _) {}); // keep alive (autoDispose)
    return c.read(chatControllerProvider('t').notifier);
  }

  test('loadInitial：desc 2 筆 → requests 反轉成 asc;initialLoading false', () async {
    final repo = _MockRepo();
    when(() => repo.fetchRequests(
          ...
          items: [
            _req(id: 2, message: 'b', status: RequestStatus.completed, reply: 'B'),
            _req(id: 1, message: 'a', status: RequestStatus.completed, reply: 'A'),
          ],
          hasMore: false,
        ));
  ...
  test('send：樂觀 temp → processing → poll completed → reply 可見 + tripDays invalidate',
      () async {
    final repo = _MockRepo();
    when(() => repo.fetchRequests(
          tripId: any(named: 'tripId'),
  ...
  test('send：403 → error 提示 + 移除 temp', () async {
    ...
    when(() => repo.sendRequest(
        ...
        const ApiError(status: 403, code: 'FORBIDDEN', message: 'no'));
  ...
  test('messages：completed row 投影成 user + assistant 兩氣泡', () async {
    ...
    when(() => repo.fetchRequests(
          ...
          items: [
            _req(id: 1, message: 'q', status: RequestStatus.completed, reply: 'a'),
          ],
          hasMore: false,
        ));
