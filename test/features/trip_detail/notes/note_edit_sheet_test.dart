...
Future<void> _open(
  ...
  await tester.pumpWidget(ProviderScope(
    overrides: [
      ...
      tripNotesProvider('t1').overrideWith((ref) async => const TripNotes()),
    ],
    child: MaterialApp(
...
void main() {
  ...
  testWidgets('create reservations：填欄位 + 選 enum → createNote', (tester) async {
    ...
    final fields = verify(() => repo.createNote(NoteSection.reservations,
            tripId: 't1',
  ...
  testWidgets('create reservations：未動 kind → 預設 restaurant', (tester) async {
    ...
    final fields = verify(() => repo.createNote(any(),
            tripId: any(named: 'tripId'),
            fields: captureAny(named: 'fields')))
        .captured
        .single as Map<String, dynamic>;
  ...
  testWidgets('edit reservations：預填 + 改 title → updateNote(rowId, expectedVersion)',
    ...
    when(() => repo.updateNote(any(),
        ...
        expectedVersion: any(named: 'expectedVersion'))).thenAnswer((_) async {});
    await _open(tester, repo, section: NoteSection.reservations, initialFields: const {
      'kind': 'restaurant',
      'title': '舊',
      'reserved_at': '',
      'party_size': 2,
    ...
    await tester.enterText(find.byKey(const ValueKey('note-field-title')), '新');
    await tester.pump();
    await tester.tap(find.byKey(const ValueKey('note-edit-submit')));
    await tester.pumpAndSettle();

    final fields = verify(() => repo.updateNote(NoteSection.reservations,
            tripId: 't1',
            rowId: 5,
            fields: captureAny(named: 'fields'),
            expectedVersion: 3))
