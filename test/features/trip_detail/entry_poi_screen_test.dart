...
Future<void> _pump(WidgetTester tester, _MockTripRepository repo,
    {_MockPoiRepository? poiRepo}) async {
  await tester.pumpWidget(ProviderScope(
    overrides: [
      tripRepositoryProvider.overrideWithValue(repo),
      entryDetailProvider((tripId: 't1', entryId: 11))
          .overrideWith((ref) async => _entry),
...
void main() {
  ...
  testWidgets('設為正選 → setEntryMaster(poiId, entryPoisVersion)', (tester) async {
    ...
    verify(() => repo.setEntryMaster(
        tripId: 't1', entryId: 11, poiId: 502, entryPoisVersion: '4')).called(1);
  });

  testWidgets('移除備選 → removeEntryAlternate', (tester) async {
    final repo = _MockTripRepository();
    when(() => repo.removeEntryAlternate(
          tripId: any(named: 'tripId'),
          entryId: any(named: 'entryId'),
          poiId: any(named: 'poiId'),
          entryPoisVersion: any(named: 'entryPoisVersion'),
    ...
    verify(() => repo.removeEntryAlternate(
        tripId: 't1', entryId: 11, poiId: 502, entryPoisVersion: '4')).called(1);
  });

  testWidgets('編輯資訊 → 改備註 → 儲存呼叫 updateEntryPoi', (tester) async {
    final repo = _MockTripRepository();
    when(() => repo.updateEntryPoi(
          tripId: any(named: 'tripId'),
          entryId: any(named: 'entryId'),
          poiId: any(named: 'poiId'),
          note: any(named: 'note'),
    ...
    verify(() => repo.updateEntryPoi(
        tripId: 't1',
        entryId: 11,
        poiId: 501,
        note: '記得拍照',
        poiType: 'attraction',
        reservation: any(named: 'reservation'))).called(1);
  });

  testWidgets('加入備選 → 搜尋選結果 → addEntryAlternate', (tester) async {
    final repo = _MockTripRepository();
    final poiRepo = _MockPoiRepository();
    when(() => repo.addEntryAlternate(
          tripId: any(named: 'tripId'),
          entryId: any(named: 'entryId'),
          poi: any(named: 'poi'),
          entryPoisVersion: any(named: 'entryPoisVersion'),
        )).thenAnswer((_) async {});
    when(() => poiRepo.searchPois(
          q: any(named: 'q'),
    ...
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const ValueKey('alt-search-field')), '拉麵');
    await tester.tap(find.byKey(const ValueKey('alt-search-button')));
    await tester.pumpAndSettle();
    await tester.tap(find.byKey(const ValueKey('alt-result-p9')));
    await tester.pumpAndSettle();

    verify(() => repo.addEntryAlternate(
        tripId: 't1',
        entryId: 11,
        poi: any(named: 'poi'),
        entryPoisVersion: '4')).called(1);
  });
}
