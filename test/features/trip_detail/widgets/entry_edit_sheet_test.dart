...
Future<void> _open(
    WidgetTester tester, _MockTripRepository repo, EntryEditArgs args) async {
...
void main() {
  ...
  group('entryTimeRangeValid', () {
    test('皆 null / 任一 null → true', () {
      ...
      expect(entryTimeRangeValid(const TimeOfDay(hour: 9, minute: 0), null),
          isTrue);
      expect(entryTimeRangeValid(null, const TimeOfDay(hour: 9, minute: 0)),
          isTrue);
    });
    test('end > start → true；end <= start → false', () {
      expect(
          entryTimeRangeValid(const TimeOfDay(hour: 9, minute: 0),
              const TimeOfDay(hour: 10, minute: 0)),
          isTrue);
      expect(
          entryTimeRangeValid(const TimeOfDay(hour: 10, minute: 0),
              const TimeOfDay(hour: 10, minute: 0)),
          isFalse);
      expect(
          entryTimeRangeValid(const TimeOfDay(hour: 11, minute: 0),
              const TimeOfDay(hour: 10, minute: 0)),
          isFalse);
    });
  });
  ...
  testWidgets('新增模式：送出呼叫 addEntryToDay(source custom)', (tester) async {
    ...
    await tester.enterText(
        find.byKey(const ValueKey('entry-edit-title')), '自由活動');
    await tester.pump();
    await tester.tap(find.byKey(const ValueKey('entry-edit-submit')));
    await tester.pumpAndSettle();
