...
void main() {
  ...
  test('GET noStore → 有快取也不回退', () async {
    await cache.writeResponse(
      cacheKeyFor('GET', '/invitations', {'token': 'raw-token'}),
      {'tripId': 'stale-trip'},
    );
  ...
  group('讀取政策', () {
    test('networkOnly → GET 成功後仍寫入快取', () async {
      final key = cacheKeyFor('GET', '/trips/t1');
      adapter.onGet('/trips/t1', (s) => s.reply(200, {'id': 'fresh'}));

      await client.get('/trips/t1', policy: CacheReadPolicy.networkOnly);

      expect((await cache.readResponse(key))?.data, {'id': 'fresh'});
    });
