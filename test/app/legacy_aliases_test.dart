...
void main() {
  ...
  test('帳號 sheet:來源頁 + account=page;去掉 account 就回來源頁', () {
    expect(
      accountSheetLocation(
        Uri.parse('/account/sessions?account_origin=%2Ffavorites%3Fq%3D1'),
        'sessions',
      ),
      '/favorites?q=1&account=sessions&account_origin=%2Ffavorites%3Fq%3D1',
    );
    expect(
      withoutAccount(
        Uri.parse(
    ...
    expect(
      accountSheetLocation(
        Uri.parse('/account?account_origin=https%3A%2F%2Fevil.example'),
        'root',
      ),
      '/trips?account=root&account_origin=%2Ftrips',
    );
  });
  ...
  test('alias 表:每一行的目標都釘住;表就是規格,少一行或改錯都要紅', () {
    ...
    final expected = <String, String>{
      ...
      '/trip/:tripId/map': '/map?place_id=p1&tripId=t+1',
