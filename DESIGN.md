...
- **Tripline 決策**：#319 以 iPhone 14 Pro／iOS 16.6 真機對照 Apple Music 參考圖後，只有四項明文偏離套件預設，其餘光學參數不動：
  ...
  2. 選單面板使用 `tpMenuGlassSettings` 與 blur 24，避免後方標題字形干擾。2026-09-15 依使用者提供的 root tab bar 參考圖，深色底色改與導覽玻璃共用（目前白色 8%），移除原先額外加亮至 18% 的白膜；淺色維持白色 72%。保留既有排列、分組、圓角與共用邊緣光；提高對比／降低透明度各自使用 `surfaceContainerHigh` 不透明底。#319 舊配方的量測保留於 ADR-0004，不作為新底色的驗收證據。選單目前不開在媒體背景上。
