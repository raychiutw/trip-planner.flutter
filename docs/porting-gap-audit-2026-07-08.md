...
| `/trip/:id/map` → `MapPage` | `/trips/:id/map` + `/trips/:id/stop/:entryId/map` → `TripMapScreen` | 部分翻 | 已補 entry focus route、overview 點 pin 切 day；續補 pin/card 雙向同步、路線/定位控制 |
...
- **TripMapScreen**：目前是 OSM pins + day tabs + entry cards；`stop/:entryId/map` 會初載切到 entry 所在 day 並聚焦該 pin，總覽點 pin 也會自動切到該 pin 所在 day。尚缺 pin/card 雙向同步、路線 polyline、圖層/我的位置 FAB。
