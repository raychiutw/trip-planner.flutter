...
| `/trip/:id` embedded `TripPage` | `/trips/:id` → `TripTimelineScreen` | 部分翻 | 已補 focus/deeplink、scroll-spy；續補 action menu、segment 背景同步細節 |
| `/trip/:id/map` → `MapPage` | `/trips/:id/map` + `/trips/:id/stop/:entryId/map` → `TripMapScreen` | 部分翻 | 已補 entry focus route；續補 pin/card 雙向同步、路線/定位控制 |
...
- **TripTimelineScreen**：目前有 day pills + timeline + travel pill + map/notes/health/add-entry actions；travel pill 會讀取 `trip_segments` source of truth 並可編輯 driving/walking/transit；`?focus=<entryId>` 可初載定位指定 entry，手動捲動也會以 scroll-spy 同步 active day。尚缺今日自動定位、offline banner、segment 背景同步細節、行程切換 dropdown、overflow actions。
- **TripMapScreen**：目前是 OSM pins + day tabs + entry cards；`stop/:entryId/map` 會初載切到 entry 所在 day 並聚焦該 pin。尚缺 pin/card 雙向同步、overview 點 pin 自動切 day、路線 polyline、圖層/我的位置 FAB。
