...
| `/trips` → `TripsListPage` | `/trips` → `TripsListScreen` | 部分翻 | 已補 filter/sort/search/filtered empty/menu actions/尾端新增卡/JSON 匯入；尚缺分享 |
...
- **TripsListScreen**：目前有單欄清單、下拉更新、AppBar/空狀態新增入口、分類 tabs（全部/我的/共編/已歸檔）、搜尋、排序、filtered empty、TripCard action menu（編輯/共編/AI 健檢/筆記/刪除）、尾端新增卡、JSON 匯入。尚缺 ShareLink 分享管理。
...
- 公開分享 `/s/:token`、ShareLink、列印/PDF、JSON 匯出。
