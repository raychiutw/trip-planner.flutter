...
| `/trips` → `TripsListPage` | `/trips` → `TripsListScreen` | 已翻第一波 | 已補 filter/sort/search/filtered empty/menu actions/尾端新增卡/JSON 匯入/匯出/分享連結管理 |
...
| `/account` → `AccountPage` | `/account` → `AccountScreen` | 已翻第一波 | 已補 displayName inline edit、外觀/通知/登入裝置 rows 導航 |
...
- **TripsListScreen**：目前有單欄清單、下拉更新、AppBar/空狀態新增入口、分類 tabs（全部/我的/共編/已歸檔）、搜尋、排序、filtered empty、TripCard action menu（編輯/共編/AI 健檢/筆記/匯出 JSON/分享/刪除）、尾端新增卡、JSON 匯入/匯出與 ShareLink 分享管理。
