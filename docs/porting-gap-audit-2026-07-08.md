...
| `/trip/:id/stop/:eid/edit/change-poi/copy/move` | `/trips/:id/stop/:entryId/edit` → `EditEntryScreen`; `/trips/:id/stop/:entryId/change-poi` → `ChangePoiScreen` | 部分翻 | 已補時間/描述/刪除、主景點置換、加備選 slice；copy/move/備選移除排序待補 |
...
2. **Trip action surface**
   ...
   - 狀態：`/trips/:id/add-entry` / `add-stop` 已有搜尋/收藏新增 slice；`/trips/:id/stop/:entryId/edit` 已有時間/描述/刪除 slice；`/trips/:id/stop/:entryId/change-poi` 已有主景點置換/加備選 slice；自訂景點、copy/move、segments edit 仍待補。
...
3. **Entry detail/action forms**
   ...
   - 狀態：`stop/:entryId/edit` 已先補讀取、時間/描述更新與刪除；`change-poi` 已補搜尋/收藏置換 master 與新增 alternate；備選移除/排序、copy/move 與 409 `STALE_ENTRY` 重抓再套用仍待補。
...
下一個 Build branch 建議接 **Entry copy/move/備選管理** 或 **建立/編輯行程**。Favorites / Explore / 加入行程 fast-path 第一波已把 primary tab placeholder 轉正；Entry CRUD 已先補新增景點搜尋/收藏、edit 時間/描述/刪除、change-poi 主景點置換/加備選 slice。

1. 若續做 Entry CRUD：優先補備選移除/排序、copy/move，OCC 與 409 `STALE_ENTRY` handling 要同步進測試。
