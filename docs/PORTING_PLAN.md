...
P1（第二波）：收藏 + Explore + 加入行程 fast-path 已完成第一波；Entry CRUD 已完成 `/trips/:id/add-entry` 搜尋/收藏新增 slice 與 `/trips/:id/stop/:entryId/edit` 時間/描述/刪除 slice，change-poi/copy/move、自訂地圖 picker、409 `STALE_ENTRY` 重抓再套用、建立/編輯行程、聊天（request queue）、全域地圖、共編仍待辦。
