...
| CollabScreen / InviteScreen | CollabPage / InvitePage（共編邀請第一波） | `GET/POST /permissions`、`GET /invitations?tripId=...`、`POST /invitations/revoke`、`GET /invitations?token=...`、`POST /invitations/accept` |
...
lib/
  ...
  models/                   # trip / day / entry / poi / chat / collab / notes / user…（fromJson + 等值）
  ...
  api/trip_repository.dart  # my-trips / trips / days / requests / permissions / invitations / notes / stats
