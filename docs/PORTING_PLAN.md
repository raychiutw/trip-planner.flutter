...
| TripsListScreen | TripsListPage（分類/搜尋/排序/action menu 第一波） | `GET /my-trips` rich summary（client-side filter/sort/search）、`DELETE /trips/:id`、現有 trips child routes |
...
| TripTimelineScreen | TripPage（embedded） | `GET /trips/:id` + `GET /trips/:id/days?all=1` + `GET/PATCH /trips/:id/segments` |
