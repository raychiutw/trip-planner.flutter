...
| TripMapScreen | MapPage | 共用 trip scope 資料（不重抓）；支援 `stop/:entryId/map` focus route、overview pin → day 切換、pin-card selected sync 與 day route polyline |
