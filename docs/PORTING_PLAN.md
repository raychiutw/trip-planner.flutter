...
| TripTimelineScreen | TripPage（embedded） | `GET /trips/:id` + `GET /trips/:id/days?all=1` + `GET/PATCH /trips/:id/segments`；支援 `?focus=<entryId>` 初載定位、今日自動定位、scroll-spy active day 與 overflow actions |
