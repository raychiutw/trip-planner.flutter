...
| `/trips/:tripId/entries/new?day=N&mode=search|favorites|custom&region=沖繩` | `EntryAddRouteScreen` | tab 2 孫路由(搜尋 POI / 收藏景點 / 自訂停留點；搜尋模式可帶 region,加入前 resolve Place Details 產生營業/消費/地址備註；搜尋/收藏可分類篩選與多選批次加入) |
