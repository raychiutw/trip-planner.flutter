...
`edgeAbsorption`，無障礙降級仍歸零。兩個新值在當時（2026-09-11 決定時）仍是公式推估，待 0.26.8
真機重量測四邊峰值才算驗收。證據與推估腳本在 #319 evidence 的 `specular-final/`。

同日稍後 0.26.8（`816ef76`，run 34591742975）同裝置、同量法重量測：header 圓鈕 +47／+33／+43／+32、
標題膠囊 +45／+43／+30、root tab bar +71／+68／+52。header 落在推估範圍；tab bar 高於推估的
+58／+44，但最亮側已到參考頂緣 +66 量級，全周環轉為方向性、無黑框。側邊殘環是套件雙向 lobe
寫死的公開能力極限，已接受，不再加光照修正。淺色白底 +1～+4 無新增暗框。此段只量邊緣高光；
同 run 發現媒體上的淺色標題文字漏套媒體前景（2.6:1），至少 0.26.6 已存在的既有契約缺陷，由 #319 的
0.26.9 修正，紀錄在 [1.4.1 遷移紀錄](../liquid-glass-1.4.1-migration.md)。

### 深色選單底色依使用者參考圖更正（2026-09-15）

使用者要求圖 1 的行程操作選單參考圖 2 的樣式；圖 2 可見的 root tab bar 採
較深且通透的玻璃。此次以 root tab bar 為材質基準，深色選單改共用導覽底色
（目前白色 8%），取代 #319 的 18% 白膜，減少整片偏灰白的外觀。
文字背景仍保留 blur 24；淺色白色 72%、快捷動作與分組、圓角、共用邊緣光、
提高對比及降低透明度的獨立不透明容器降級維持原契約。

此段只取代前述深色選單底色決策。#319 的量測屬於舊配方，不能轉作新配方的
驗收結果；新版須另取實體 iOS 裝置畫面核對材質與背景文字干擾。
