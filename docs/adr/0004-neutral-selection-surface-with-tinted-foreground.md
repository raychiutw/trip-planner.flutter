...
| | 填色 | 邊緣峰值 | 高出 |
|---|---|---|---|
| iOS 26 訊息 標題膠囊 | `#141414` | `#333333` | **+31** |
| iOS 26 電話 編輯膠囊 | `#141414` | `#313131` | **+29** |
| Tripline(現況) | `#0D0D0D` | `#848584` | **+119** |

我們是 Apple 的 **4 倍**。降 `ambientRim` / `glowIntensity` 到差值 ≈ +30。

**注意這條亮邊不是畫上去的。** `AppleRootTabBar` 沒有任何 `BorderSide`,
`tpGlassEdgeColor` 在一般模式回傳透明 —— 它是 LiquidGlass 材質的環境邊緣光。
修法是調材質參數,不是移除描邊。這一點是 [ADR-0001](0001-keep-liquid-glass-over-native-cupertino.md)
「邊緣交還材質」方向的延續與校準,不是推翻。
...
本 ADR 的每一個數字都來自對 Apple 截圖的像素量測,而不是目視判斷。這是刻意的 ——
這條線上前後兩份文件(#118 與 ADR-0003)都因為「看起來像」而下了錯誤結論。
往後對齊 Apple 的視覺決策,先量再寫。
